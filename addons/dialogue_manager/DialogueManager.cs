...
namespace DialogueManagerRuntime
{
    ...
    public partial class DialogueManager : RefCounted
    {
        ...
        public static GodotObject Instance
        {
            get
            {
                if (instance == null)
                {
                    ...
                    instance.Connect(
                        "bridge_dialogue_started",
                        Callable.From(
                            (Resource dialogueResource) => DialogueStarted?.Invoke(dialogueResource)
                        )
                    );
                }
                return instance;
        ...
        {
            return (Array<string>)
                Instance.Call("static_id_to_line_ids", dialogueResource, staticId);
        }
        ...
        public static Array<Dictionary> GetMembersForAutoload(Script script)
        {
            ...
            var matchingTypes = Assembly
                .GetExecutingAssembly()
                .GetTypes()
                .Where(t => t.Name == typeName);
            foreach (var matchingType in matchingTypes)
            {
                var memberInfos = matchingType.GetMembers(
                ...
                foreach (var memberInfo in memberInfos)
                {
                    ...
                    members.Add(new Dictionary() { { "name", memberInfo.Name }, { "type", type } });
                }
            }
        ...
        public Variant ResolveThingConstant(GodotObject thing, string property)
        {
            ...
            foreach (var fieldInfo in fieldInfos)
            {
                if (fieldInfo.Name == property && fieldInfo.IsLiteral)
                {
                    ...
                    {
                        throw new Exception(
                            $"Constant {property} of type ${fieldInfo.GetValue(thing).GetType()} is not supported by Variant."
                        );
                    }
                }
            }
        ...
        public bool ThingHasMethod(GodotObject thing, string method, Array<Variant> args)
        {
            ...
            foreach (var methodInfo in methodInfos)
            {
                if (
                    ...
                    && args.Count
                        >= methodInfo.GetParameters().Where(p => !p.HasDefaultValue).Count()
                )
                {
                    return true;
                }
